...
namespace Supercent.UI
{
    ...
    /// </summary>
    public abstract class CurrencyHUDBase_Long : MonoBehaviour
    {
        class HUDInfos
        {
            public int                      LastRefreshFrame    = 0;
            public List<CurrencyHUDBase_Long> List              = new List<CurrencyHUDBase_Long>();
        }
        ...
        void OnEnable() 
        {
            ...
            if (null == info)
            {
                Debug.LogWarning($"[CurrencyHUDBase_Long - OnEnable] {name}이 비정상적인 상태입니다.");
                return;
            }
        ...
        public static CurrencyHUDBase_Long GetFocusedHUD(Type key)
        {
        ...
        protected abstract void RegistEvent(Action<long> callback);
        protected abstract void UnregistEvent(Action<long> callback);



        protected void RefreshAll_CurrentType(long value) => RefreshAll(GetType(), value, false);
        public static void RefreshAll(Type key, long value, bool force)
        {
            var info = AddGetHUDInfos(key);
            if (null == info)
                return;
        ...
        public void Refresh(long value, bool useSkipAnim)   => OnRefresh(value, useSkipAnim);
        void OnRefresh(long value, bool useSkipAnim)
        {
            if (useSkipAnim)
            {
                StopAllCoroutines();
        ...
        {
            var currentValue = Math.Max(0, Calc_CurrentValue());
            if (null != _valueTMP)
                _valueTMP.text = ValueToString(currentValue);

            _ApplyValue_ForUI(currentValue);
        }
        protected abstract void _ApplyValue_ForUI(long value);



        protected abstract long GetCurrencyValue_FromUserData();
        protected virtual string ValueToString(long value) => $"{value:0}";
        public long Calc_CurrentValue()
        {
...
#if UNITY_EDITOR
    [UnityEditor.CustomEditor(typeof(CurrencyHUDBase_Long), true)]
    [UnityEditor.CanEditMultipleObjects]
    [ExecuteInEditMode]
    public class CurrencyHUDBase_LongEditor : UnityEditor.Editor
    {
        public static readonly MethodInfo BIND_SERIALIZED_FIELD = typeof(CurrencyHUDBase_Long).GetMethod("BindSerializedField", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Bind Serialized Field"))
            {
                ...
                {
                    var target = targetList[n] as CurrencyHUDBase_Long;
                    if (null == target)
                        continue;
        ...
        void OnSceneGUI() 
        {
            ...
            var bb = target as CurrencyHUDBase_Long;
            bb.OnSceneGUI();
        }
    }
#endif // UNITY_EDITOR
